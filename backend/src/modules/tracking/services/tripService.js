...
class TripService {
  /**
   * Retrieves active trip for bus, enforcing 3-hour duration limit safeguard.
   */
  ...
  async startTrip({ busId, routeId, driverId, direction = 'morning' }) {
    ...
    const trip = await Trip.create({
      ...
      driverId: driverId || bus.driverId || (bus.assignedDriverId ? bus.assignedDriverId.toString() : null),
      direction,
      status: 'in_progress',
      startedAt: new Date(),
    });
